...
class Acadamic_year(models.Model):
    ...
    name = fields.Char("Name", required=True, help="Name of Session")
    code = fields.Char("Code", required=True, help="Code of Session")
    date_start = fields.Date(
        "Start Date", required=True, help="Starting date of Session"
    )
    date_stop = fields.Date(
        "End Date", required=True, help="Ending of Session"
    )
    ...
    active = fields.Boolean("Active", help="Set Active Session")
    ...
    def _check_academic_year(self):
        ...
        if delta.days > 365 and not calendar.isleap(new_start_date.year):
            raise ValidationError(
                _("The duration of the Session is invalid.")
            )
        if (
            self.date_stop
            and self.date_start
            ...
            raise ValidationError(
                _(
                    "The start date of the Session should be less than "
                    "end date."
                )
            )

class AcademicMonth(models.Model):
    """Defining a month of an Session."""
...
class CentreStandard(models.Model):
    """Defining a standard related to centre."""

    _name = "centre.standard"
    _description = "centre Standards"
    _rec_name = "standard_id"

    @api.depends(
        "standard_id", "centre_id", "division_id", "medium_id", "centre_id"
    )
    ...
    centre_id = fields.Many2one(
        ...
        "centre",
        required=True,
        help="centre of the following standard",
    )
    ...
    user_id = fields.Many2one(
        "centre.teacher", "Class Teacher", help="Teacher of the standard"
    )
    ...
    cmp_id = fields.Many2one(
        "res.company",
        "Company Name",
        related="centre_id.company_id",
        store=True,
        help="Company_id of the centre",
    )
    ...
    def check_standard_unique(self):
        ...
        if self.env["centre.standard"].search(
            [
            ...
            raise ValidationError(_("Division and Group should be unique!"))
    ...
    def unlink(self):
        ...
        return super(CentreStandard, self).unlink()
...
class CentreCentre(models.Model):
    """ Defining centre Information"""

    _name = "centre.centre"
    _description = "centre Information"
    _rec_name = "com_name"

    @api.constrains("code")
    def _check_code(self):
        for record in self:
            if self.env["centre.centre"].search(
                [("code", "=", record.code), ("id", "!=", record.id)]
            ):
                raise ValidationError(_("centre Code must be Unique"))
    ...
    code = fields.Char("Code", required=True, help="centre code")
    standards = fields.One2many(
        "centre.standard", "centre_id", "Standards", help="centre standard"
    )
    ...
    def create(self, vals):
        """Inherited create method to assign company_id to centre"""
        res = super(CentreCentre, self).create(vals)
...
class SubjectSubject(models.Model):
    ...
    standard_id = fields.Many2one(
        "standard.standard",
        "Group",
        help="""Group in which the following
                                  subject taught""",
    )
...
class SubjectSyllabus(models.Model):
    ...
    standard_id = fields.Many2one(
        "centre.standard", "Standard", help="Standard which had this subject"
    )
...
class StudentDescipline(models.Model):
    ...
    teacher_id = fields.Many2one(
        "centre.teacher", "Teacher", help="Teacher who examine the student"
    )
    date = fields.Date("Date", help="Date")
    group_id = fields.Many2one(
        "standard.standard", "Group", help="Group of student"
    )
...
class StudentHistory(models.Model):
    ...
    year_id = fields.Many2one(
        "academic.year", "Session", help="Session"
    )
    standard_id = fields.Many2one(
        "centre.standard", "Standard", help="Standard of the following student"
    )
...
class StudentPreviouscentre(models.Model):
    """ Defining a student previous centre information """

    _name = "student.previous.centre"
    _description = "Student Previous centre"

    previous_centre_id = fields.Many2one(
        "student.student", "Student", help="Related student"
    )
    name = fields.Char(
        "Name", required=True, help="Student previous centre name"
    )
    ...
    exit_date = fields.Date(
        "Exit Date", help="Student previous centre exit date"
    )
    ...
    def check_date(self):
        ...
        if (self.admission_date and self.exit_date) and (
            ...
            raise ValidationError(
                _(
                    "Admission date should be less than exit date in previous centre!"
                )
            )
...
class AcademicSubject(models.Model):
    ...
    add_sub_id = fields.Many2one(
        "student.previous.centre",
        "Add Subjects",
        invisible=True,
        help="Select student previous centre",
    )
    name = fields.Char(
        "Name", required=True, help="Enter previous centre name"
    )
