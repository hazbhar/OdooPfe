...
class StudentStudent(models.Model):
    ...
    def _compute_teacher_user(self):
        ...
        teacher = self.env.user.has_group("IDAFormation.group_centre_teacher")
        for rec in self:
    ...
    def check_current_year(self):
        ...
        res = self.env["academic.year"].search([("active", "=", True)])
        if not res:
            raise ValidationError(
                _(
                    "There is no Session defined! Please "
    ...
    previous_centre_ids = fields.One2many(
        "student.previous.centre",
        "previous_centre_id",
        "Previous centre Detail",
        states={"done": [("readonly", True)]},
        help="Enter student centre details",
    )
    remark = fields.Text(
        "Remark",
        states={"done": [("readonly", True)]},
    ...
    def create(self, vals):
        ...
        if vals.get("email"):
            centre.emailvalidation(vals.get("email"))
        res = super(StudentStudent, self).create(vals)
        teacher = self.env["centre.teacher"]
        # Assign group to student based on condition
        emp_grp = self.env.ref("base.group_user")
        if res.state == "draft":
            admission_group = self.env.ref("IDAFormation.group_is_admission")
            new_grp_list = [admission_group.id, emp_grp.id]
            res.user_id.write({"groups_id": [(6, 0, new_grp_list)]})
        elif res.state == "done":
            done_student = self.env.ref("IDAFormation.group_centre_student")
    ...
    def check_age(self):
        ...
        if self.date_of_birth:
            ...
            if age_calc < self.centre_id.required_age:
                raise ValidationError(
                    _(
                        "Age of student should be greater than %s years!"
                        % (self.centre_id.required_age)
                    )
                )
    ...
    def admission_done(self):
        ...
        centre_standard_obj = self.env["centre.standard"]
        ir_sequence = self.env["ir.sequence"]
        student_group = self.env.ref("IDAFormation.group_centre_student")
        emp_group = self.env.ref("base.group_user")
        for rec in self:
            if not rec.standard_id:
                raise ValidationError(_("Please select Group!"))
            if rec.standard_id.remaining_seats <= 0:
                raise ValidationError(
                    _("Seats of Group %s are full")
                    % rec.standard_id.standard_id.name
                )
            domain = [("centre_id", "=", rec.centre_id.id)]
            # Checks the standard if not defined raise error
            if not centre_standard_obj.search(domain):
                raise UserError(
                    _("Warning! The standard is not defined in centre!")
                )
            ...
            registation_code = (
                str(rec.centre_id.state_id.name)
                + str("/")
                + str(rec.centre_id.city)
                + str("/")
                + str(rec.centre_id.name)
                + str("/")
                + str(reg_code)
            )
            stu_code = ir_sequence.next_by_code("student.code")
            student_code = (
                str(rec.centre_id.code)
                + str("/")
                + str(rec.year.code)
                + str("/")
                + str(stu_code)
            ...
            template = (
                ...
                .sudo()
                .search([("name", "ilike", "Inscription Confirmation")], limit=1)
            )
            if template:
                for user in rec.parent_id:
                    subject = _("About Inscription Confirmation")
                    if user.email:
                        body = (
                            """
                        <div>
