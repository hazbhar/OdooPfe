...
class ReportMonthAttendace(models.AbstractModel):
    ...
    def get_dates(self, rec):
        days_of_month = calendar.monthrange(int(rec.academic_year_id.code),
                                            int(rec.month))[1]
    ...
    def get_data(self, rec):
        ...
        if int(rec.month) < 10:
            month = '0' + rec.month
        last_day_month = calendar.monthrange(int(rec.academic_year_id.code),
                                                 int(month))[1]
        start_date_str = (str(int(rec.academic_year_id.code)) + '-' +
                          str(int(month)) + '-01')
        end_date_str = (str(int(rec.academic_year_id.code)) + '-' +
                          str(int(month)) + '-' + str(last_day_month) + ' 23:00:00')
        self._cr.execute('''
            SELECT
                id
            FROM
        ...
        for att in self.env['daily.attendance'].browse(records):
            ...
            if not group_data:
                group_data.append({
                    'user': att.user_id,
                    'centre_name': att.user_id.centre_id.name,
                    ...
                    if gdata.get('user').id == att.user_id.id:
                        ...
                        for att_id in gdata.get('att_ids'):
                            if att_id.get('date') == day_date:
                                ...
                                break
                        if not flag_date:
                            gdata.get('att_ids').append({'date': day_date,
                                                         'att': [att]})
                if not flag:
                    group_data.append({
                        'user': att.user_id,
                            ...
                            if day_date == att_count:
                                ...
                                if student.is_present:
                                    ...
                                    if day_date in matched_dates or not matched_dates:
                                        if att_data.get(student.stud_id.name) and att_data.get(student.stud_id.name).get(att_count):
                                            if att_data.get(student.stud_id.name).get(att_count) != 'A':
                                                status = int(att_data.get(student.stud_id.name).get(att_count)) + no_of_class
                                else:
                                    if day_date in matched_dates:
                                        if att_data.get(student.stud_id.name) and att_data.get(student.stud_id.name).get(att_count):
                                            if att_data.get(student.stud_id.name).get(att_count) != 'A':
                                                status = int(att_data.get(student.stud_id.name).get(att_count))
                                if not att_data.get(student.stud_id.name):
                                    att_data.update({student.stud_id.name: {att_count: str(status)}})
                                    total_absent = 0
                                    ...
                                    att_data.get(student.stud_id.name).update({att_count: str(status)})
                                    for stu in data:
                                ...
                                status = ''
                                if not att_data.get(student.stud_id.name):
                                    att_data.update({student.stud_id.name: {att_count: status}})
                                    data.append({'roll_no': student.stud_id.roll_no,
                                                 'student_code': student.stud_id.student_code,
                                                 'total_absent': 0,
                                                 'name': student.stud_id.name,
                                                 'att': {att_count: status}})
                                else:
                                    if att_data.get(student.stud_id.name).get('att_count') == '':
                                        att_data.get(student.stud_id.name).update({att_count: status})
                                        for stu in data:
                                            if stu.get('name') == student.stud_id.name:
            ...
            for roll_no in roll_no_list:
                for stu in data:
                    if stu.get('roll_no') == roll_no:
                        ...
                        data.remove(stu)
            res_data.append({'user': gdata.get('user').name,
                             'centre_name': gdata.get('centre_name'),
    ...
    def get_total_class(self, rec):
        ...
        last_day_month = calendar.monthrange(int(rec.academic_year_id.code),
                                                 int(rec.month))[1]
        start_date_str = (str(int(rec.academic_year_id.code)) + '-' +
                          str(int(rec.month)) + '-01')
        end_date_str = (str(int(rec.academic_year_id.code)) + '-' +
                          str(int(rec.month)) + '-' + str(last_day_month) + ' 23:00:00')
        self._cr.execute('''
            SELECT
                id
            FROM
