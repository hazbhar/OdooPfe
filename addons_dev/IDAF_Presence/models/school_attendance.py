...
class AttendanceSheet(models.Model):
    ...
    standard_id = fields.Many2one('IDAFormation.standard', 'Academic Class',
    ...
    attendance_ids = fields.One2many('attendance.sheet.line', 'standard_id',
        'Attendance', help="Academic Year")
    user_id = fields.Many2one('IDAFormation.teacher', 'Faculty',
        help="Select Teacher")
    ...
    def fields_view_get(self, view_id=None, view_type='form',
        ...
        if view_type == 'form':
            digits_temp_dict = {1: 'one', 2: 'two', 3: 'three', 4: 'four',
                                ...
                                9: 'nine', 10: 'ten', 11: 'one_1',
                                12: 'one_2', 13: 'one_3', 14: 'one_4',
                                15: 'one_5', 16: 'one_6', 17: 'one_7',
                                18: 'one_8', 19: 'one_9', 20: 'one_0',
            ...
            if st_dates and end_dates:
                while st_dates <= end_dates:
                    res['fields']['attendance_ids'
                        ...
                        ][digits_temp_dict.get(flag)]['string'] = st_dates.day
                    st_dates += rd(days=1)
                    flag += 1
            if flag < 32:
                res['fields']['attendance_ids'
                    ]['views']['tree']['fields'][digits_temp_dict.get(flag)
                                            ]['string'] = ''
                doc2 = etree.XML(res['fields']['attendance_ids'
                    ]['views']['tree']['arch'])
...
class StudentleaveRequest(models.Model):
    ...
    standard_id = fields.Many2one('IDAFormation.standard', 'Class', required=True)
    attachments = fields.Binary('Attachment')
    state = fields.Selection([('draft', 'Draft'), ('toapprove', 'To Approve'),
        ('reject', 'Reject'), ('approve', 'Approved')], 'Status',
        default='draft', tracking=True)
    start_date = fields.Date('Start Date')
    end_date = fields.Date('End Date')
    teacher_id = fields.Many2one('IDAFormation.teacher', 'Class Teacher')
...
class DailyAttendance(models.Model):
    ...
    student_ids = fields.One2many('daily.attendance.line', 'standard_id',
            'Students', states={'validate': [('readonly', True)],
    ...
    total_student = fields.Integer(compute="_compute_total", store=True,
            help="Total Students in class", string='Total Students')
    total_presence = fields.Integer(compute="_compute_present",
            store=True, string='Present Students', help="Present Student")
    total_absent = fields.Integer(compute="_compute_absent", store=True,
            string='Absent Students', help="Absent Students")
    is_generate = fields.Boolean("Generate?")

    _sql_constraints = [('attend_unique', 'unique(standard_id,user_id,date)',
    ...
    def get_students(self):
        ...
        for rec in self:
            ...
            if rec.student_ids:
                ...
                raise ValidationError(_(
                    "No Students are found for selected criteria!"))
...
class DailyAttendanceLine(models.Model):
    ...
    def action_absent(self):
        for rec in self:
            if rec.standard_id.state == 'validate' and not rec.standard_id.is_generate:
